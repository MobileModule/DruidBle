...
/**
 * Created by dale on 19/10/15.
 */
public class CRCUtil {

    public static final int[] CRC8_TAB = { // X^8 + X^2 + X^1 + 1
    ...
    public static final int crc8(byte[] data, int len) {
        int crc = 0;
        for (int i = 0; i < len; i++) {
            byte b = data[i];
    ...
    public static final int crc8(byte[] data) {
        return crc8(data, data.length);
    }

    private static int CRC16_TAB[] = {
    ...
    public static int crc16(byte[] message, int len) {
        int crc = 0xffff;
        for (int i = 0; i < len; i++) {
            crc = (crc << 8) ^ CRC16_TAB[((crc >> 8) ^ message[i]) & 0xff];
        }
        return crc;
    }

    public static int crc16(byte[] message) {
        return crc16(message, message.length);
    }

    public static int crc16_val(int crc,byte[] message,int len){
        int crc_val = 0xffff;
        for (int i = 0; i < len; i++) {
            crc_val = (crc << 8) ^ CRC16_TAB[((crc >> 8) ^ message[i]) & 0xff];
        }
        return crc_val;
    }

    public static int calc_crc16(byte[] buf, int len , int offset) {
        int crc = 0xFFFF;
        int i;
        for (i = 0; i < len; i++) {
            crc = ((crc << 8) & 0xFFFF) ^ CRC16_TAB[((crc >> 8) ^ buf[i+offset]) & 0xFF];
        }
        return crc;
    }

    public static int calc_crc16_val(int crc,byte[] buf, int len , int offset) {
//        int crc = 0xFFFF;
        int i;
        for (i = 0; i < len; i++) {
            crc = ((crc << 8) & 0xFFFF) ^ CRC16_TAB[((crc >> 8) ^ buf[i+offset]) & 0xFF];
    ...
    public static boolean check_crc16(byte[] message) {
        int crc = crc16(message, message.length - 2);
        // make sure to AND 0xff
        int crcL = message[message.length - 2] & 0xff;
        int crcH = message[message.length - 1] & 0xff;
        return (crc & 0xff) == crcL && ((crc >> 8) & 0xff) == crcH;
    }
}
