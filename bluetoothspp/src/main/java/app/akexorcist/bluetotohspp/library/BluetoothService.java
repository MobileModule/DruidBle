...
//                    tmp = device.createRfcommSocketToServiceRecord(UUID_OTHER_DEVICE);
                ...
                tmp = (BluetoothSocket) m.invoke(device, UUID_OTHER_DEVICE);
            } catch (Exception e) {
//                try {
//                    if (BluetoothService.this.isAndroid)
//                        tmp = device.createRfcommSocketToServiceRecord(UUID_ANDROID_DEVICE);
...
//                    if (readed.contains("\n"))
                    {
                        //TODO version change
//                        byte[] reply = Arrays.copyOfRange(buffer, 0, bytes - 1);
                        byte[] reply = Arrays.copyOfRange(buffer, 0, bytes);
                        mHandler.obtainMessage(BluetoothState.MESSAGE_READ, reply.length, -1, reply).sendToTarget();
                    }
                } catch (IOException e) {
                    connectionLost();
                    // Start the service over to restart listening mode
